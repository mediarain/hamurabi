...
module.exports = function(skill) {
  ...
  skill.onState('query-action', function(request) {
    ...
    if(request.intent.name === 'BuyIntent' || request.intent.name === 'SellIntent') {
      ...
      command.error = command.buyError(game,acres)
      if(command.error) return {reply: 'Actions.Errors.' + action +'.'+command.error, to: 'action'};
      command.buy = acres;
      ...
      command.error = command.feedError(game,feed);
      if(command.error) {
      ...
      command.error = command.plantError(game,plant)
      if(command.error) {
        const buyPhrase = command.buy < 0 ? 'WithSell' : command.buy > 0 ? 'WithBuy' : ''
